module Spree
  Admin::OptionTypesController.class_eval do
    def available_ad_hoc
      set_available_ad_hoc_option_types
      render layout: false
    end

    # AJAX method for selecting an existing option type and associating with the current product
    def select_ad_hoc
      option_type = OptionType.find(params[:id])
      ad_hoc_option_type = AdHocOptionType.new(option_type: option_type, position: option_type.position)
      @object.ad_hoc_option_types << ad_hoc_option_type
      redirect_to edit_admin_ad_hoc_option_type_url(ad_hoc_option_type)
    end

    private

    def set_available_ad_hoc_option_types
      selected_option_types = @object.ad_hoc_option_types.pluck(:option_type)
      @available_option_types = OptionType.where.not(id: selected_option_types)
    end
  end
end
