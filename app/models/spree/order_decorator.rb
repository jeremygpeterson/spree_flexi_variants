  ...
  module OrderDecorator
    ...
    def add_variant(variant, quantity = 1, options = {})
      ...
      if current_item
        ...
        offset_price = product_option_values.map(&:price_modifier).compact.sum + product_customizations_values.map do |product_customization|
                                                                                   product_customization.price(variant)
                                                                                 end.sum
    ...
    def merge!(order, user = nil)
      ...
      order.line_items.each do |other_order_line_item|
        ...
        current_line_item = line_items.detect do |my_li|
          my_li.variant == other_order_line_item.variant && line_item_comparison_hooks.all? do |hook|
            send(hook, my_li, other_order_line_item.serializable_hash)
          end
        end
    ...
    def matching_customizations(existing_customizations, new_customizations)
      ...
      unless existing_customizations.map(&:product_customization_type_id).sort == new_customizations.map(&:product_customization_type_id).sort
        return false
      end
